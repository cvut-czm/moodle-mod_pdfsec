...
class backup_pdfsec_activity_task extends backup_activity_task {
    ...
    protected function define_my_steps() {
        $this->add_step(new backup_pdfsec_activity_structure_step('pdfsec_structure', 'pdfsec.xml'));
    }
    ...
    static public function encode_content_links($content) {
        ...
        $search="/(".$base."\/mod\/pdfsec\/index.php\?id\=)([0-9]+)/";
        $content= preg_replace($search, '$@PDFSECINDEX*$2@$', $content);

        // Link to choice view by moduleid
        $search="/(".$base."\/mod\/pdfsec\/view.php\?id\=)([0-9]+)/";
        $content= preg_replace($search, '$@PDFSECVIEWBYID*$2@$', $content);
